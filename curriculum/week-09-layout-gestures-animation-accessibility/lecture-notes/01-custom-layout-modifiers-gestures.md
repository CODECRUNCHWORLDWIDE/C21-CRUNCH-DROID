...
```mermaid
flowchart TD
  A["padding 16dp - outermost layer"] --> B["background Blue"]
  B --> C["padding 8dp"]
  C --> D["size 40dp - innermost content"]
```
*Each modifier wraps the rest of the chain outside-in; the first listed is the outermost layer.*
...
sequenceDiagram
  participant Finger
  participant Detector as pointerInput detector
  participant State as offsetX state
  participant Layout as layout phase
  participant Screen

  Finger->>Detector: drag 80px right
  Detector->>Detector: consume event
  Detector->>State: offsetX plus equals delta
  State->>Layout: offset lambda reads offsetX
  Layout->>Screen: place card at new x
  Finger->>Detector: release finger
  Detector->>State: check threshold
```
*One drag traced through detection, consumed state, layout-phase read, and placement.*
