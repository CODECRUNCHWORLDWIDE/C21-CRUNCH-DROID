...
```mermaid
flowchart TD
  UI["UI Tree - Box wraps Icon"] --> SEM["Semantics Tree node"]
  SEM --> ROLE["role equals Button"]
  SEM --> DESC["contentDescription Delete photo"]
  SEM --> ACTION["onClick action exposed"]
  TB["TalkBack and other a11y services"] --> SEM
```
*Accessibility services read the parallel semantics tree, never the composables themselves.*
...
```mermaid
flowchart TD
  Start["User wants to dismiss the card"] --> Sighted["Sighted user swipes the card"]
  Start --> Blind["TalkBack user opens actions menu"]
  Sighted --> Detect["pointerInput detects drag past threshold"]
  Blind --> Custom["Dismiss card custom action invoked"]
  Detect --> Dismiss["onDismiss called"]
  Custom --> Dismiss
```
*Two different paths to the same outcome - the custom action is the non-gesture equivalent of the swipe.*
