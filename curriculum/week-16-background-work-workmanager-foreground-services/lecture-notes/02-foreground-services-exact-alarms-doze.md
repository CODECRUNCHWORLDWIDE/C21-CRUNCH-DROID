...
```mermaid
flowchart TD
  A["What kind of work is this?"] --> B{"Deferrable and durable?"}
  B -- Yes --> C["WorkManager"]
  B -- No --> D{"User aware and ongoing right now?"}
  D -- Yes --> E["Foreground service"]
  D -- No --> F{"Precise wall clock and user chosen?"}
  F -- Yes --> G["Exact alarm"]
  F -- No --> C
```
*The least-power decision framework: pick the weakest tool that still satisfies the requirement.*
...
```mermaid
flowchart TD
  A["Ready to schedule exact alarm"] --> B{"canScheduleExactAlarms?"}
  B -- Yes --> C["setExactAndAllowWhileIdle"]
  B -- No --> D["Fall back to inexact alarm or WorkManager"]
  D --> E["Optionally send user to Settings to grant permission"]
```
*Never assume the exact-alarm permission is held - check at runtime and have a fallback.*
