...
stateDiagram-v2
  [*] --> ENQUEUED
  ENQUEUED --> BLOCKED: waiting on prerequisite
  BLOCKED --> ENQUEUED: prerequisite succeeded
  ENQUEUED --> RUNNING: constraints and power policy met
  RUNNING --> SUCCEEDED
  RUNNING --> FAILED
  ...
  FAILED --> [*]
```
*The WorkInfo states a worker moves through, and what drives each transition.*
...
```mermaid
flowchart TD
  A["Enqueue constrained WorkRequest"] --> B["ConstraintsTracker registers a ConstraintController per constraint"]
  B --> C["Network controller"]
  B --> D["Battery controller"]
  B --> E["Storage controller"]
  C --> F{"All constraints met?"}
  D --> F
  E --> F
  F -- No --> G["Stays ENQUEUED"]
  F -- Yes --> H{"Power policy allows?"}
  H -- No --> G
  H -- Yes --> I["Moves to RUNNING - doWork executes"]
```
*The constraint pipeline: every controller must report met, and the power policy must also allow, before work runs.*
