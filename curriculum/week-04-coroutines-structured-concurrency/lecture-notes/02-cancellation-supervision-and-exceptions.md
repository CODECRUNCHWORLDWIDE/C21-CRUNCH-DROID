...
```mermaid
flowchart TD
  A["job.cancel called"] --> B["Job enters Cancelling state"]
  B --> C{"Coroutine hits a suspension point"}
  C -->|"Yes - delay, withContext, await, yield"| D["Suspension point checks Job state"]
  D --> E["Throws CancellationException"]
  E --> F["Finally blocks run, coroutine unwinds"]
  C -->|"No - tight CPU loop"| G["Cancellation ignored until loop ends"]
```
*Cancellation only takes effect at a suspension point, so a non-suspending loop never notices it.*
...
```mermaid
flowchart TD
  P1["coroutineScope"] --> F1["Child B throws"]
  F1 --> S1["Siblings A and C cancelled"]
  S1 --> R1["Whole load fails"]
  P2["supervisorScope"] --> F2["Weather widget throws"]
  F2 --> S2["News and calendar widgets keep running"]
  S2 --> R2["Failure isolated to that one widget"]
```
*coroutineScope makes one failure all-or-nothing; supervisorScope isolates it to the failing child.*
