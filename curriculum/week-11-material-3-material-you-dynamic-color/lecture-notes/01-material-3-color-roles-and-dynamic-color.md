...
```mermaid
flowchart LR
  A["Seed color"] --> B["Tonal palette 13 tones"]
  B --> C["Role assignment"]
  C --> D["Light theme tones"]
  C --> E["Dark theme tones"]
```
*A seed color becomes a tonal palette, then specific tones are assigned to roles for light and dark.*
...
```mermaid
flowchart TD
  A["dynamicColor true and SDK 31 plus"] -->|yes| B["Wallpaper derived scheme"]
  A -->|no| C["darkTheme true"]
  C -->|yes| D["Fallback dark colors"]
  C -->|no| E["Fallback light colors"]
```
*How chooseColorScheme decides between the wallpaper derived scheme and the hand tuned fallback.*
