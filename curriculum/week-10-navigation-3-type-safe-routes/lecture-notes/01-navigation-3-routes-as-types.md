...
```mermaid
flowchart LR
  A["Back stack - your state"] --> B["NavDisplay"]
  B --> C["entryProvider"]
  C --> D["Top entrys composable"]
```
*The one data flow to memorise: your back stack drives NavDisplay, which asks entryProvider for the matching composable.*
...
```mermaid
stateDiagram-v2
  [*] --> Created: entry pushed onto back stack
  Created --> Active: viewModel resolves
  Active --> Cleared: entry popped off back stack
  Cleared --> [*]
```
*A ViewModel's lifetime tracks its back-stack entry, not the Activity - created on push, cleared on pop.*
