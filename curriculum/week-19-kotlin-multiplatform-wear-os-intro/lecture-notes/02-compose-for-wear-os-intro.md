...
```mermaid
flowchart TD
  A["Wear OS surfaces"] --> B["Main activity screen - Compose"]
  A --> C["Tiles - ProtoLayout"]
  A --> D["Complications - data only"]
  A --> E["Ongoing activities"]
  B --> F["Opened least often"]
  C --> G["Swiped to dozens of times a day"]
```
*The Compose app screen is one Wear surface among several, and the least-used one.*
...
```mermaid
sequenceDiagram
  participant Repo as KtorWeatherRepository
  participant WearVM as Wear ViewModel
  participant WearUI as ForecastWearScreen
  participant PhoneVM as Phone ViewModel
  participant PhoneUI as Phone Compose UI
  Repo->>WearVM: WeatherForecast
  WearVM->>WearUI: WearForecastUiState glance subset
  Repo->>PhoneVM: WeatherForecast
  PhoneVM->>PhoneUI: ForecastUiState full detail
```
*One shared repository, two view models mapping the same domain model down to surface-shaped UI state.*
