...
```mermaid
flowchart BT
  common["core-common: Dispatchers Clock qualifiers"]
  network["core-network"]
  database["core-database"]
  auth["feature-auth"]
  feed["feature-feed"]
  app["app - HiltAndroidApp lives here"]
  common --> network
  common --> database
  network --> auth
  network --> feed
  ...
  feed --> app
```
*Dependencies flow upward toward app; bindings install in SingletonComponent so every module above can see them.*
...
```mermaid
flowchart TD
  Q1{"Does sharing this instance matter"} -->|No| U["Leave it unscoped"]
  Q1 -->|Yes| Q2{"What is the shortest lifetime that still lets sharing work"}
  Q2 --> S1["Whole app needs it - Singleton"]
  Q2 --> S2["One ViewModel needs it - ViewModelScoped"]
  Q2 --> S3["One Activity needs it - ActivityScoped"]
  S1 --> Q3{"Does it hold an Activity Context View or Bitmap"}
  Q3 -->|Yes| Fix["Leak risk - use ApplicationContext instead"]
  Q3 -->|No| Safe["Safe to scope as chosen"]
```
*The three-question procedure for deciding whether and how to scope a binding.*
