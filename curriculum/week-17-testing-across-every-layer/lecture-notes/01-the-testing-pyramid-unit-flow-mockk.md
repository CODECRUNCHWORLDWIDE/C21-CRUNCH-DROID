...
```mermaid
flowchart TD
    A["Need to isolate a collaborator"] --> B{"Stateful and reused by many tests?"}
    B -->|Yes| C["Write a fake"]
    B -->|No| D{"Just verifying one interaction?"}
    D -->|Yes| E["Use a mock"]
    D -->|No| C
```
*Deciding between a hand-written fake and a MockK mock for a collaborator.*
...
stateDiagram-v2
    [*] --> Loading
    Loading --> Content: repository succeeds
    Loading --> Error: repository fails
    Content --> [*]
    Error --> [*]
```
*The CheckoutUiState states the test drives through and asserts on, in order.*
