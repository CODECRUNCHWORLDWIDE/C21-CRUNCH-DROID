...
```mermaid
flowchart BT
    A["Small: JUnit 5, MockK, Turbine"] --> B["Medium: Robolectric DAO test"]
    B --> C["Medium: Compose UI test"]
    C --> D["Medium: Roborazzi screenshot"]
    D --> E["Large: Espresso plus Hilt smoke"]
```
*Climbing the pyramid from lecture 1's small tier up to one Espresso smoke at the tip.*
...
```mermaid
flowchart LR
    T["Espresso test"] --> H["Hilt graph"]
    H --> VM["Real ViewModel"]
    VM --> DAO["Real DAO and database"]
    H --> FN["Fake network module"]
    FN -.replaces.-> RN["Real network module"]
```
*Hilt assembles the real ViewModel and DAO but swaps in a fake network module for the smoke.*
