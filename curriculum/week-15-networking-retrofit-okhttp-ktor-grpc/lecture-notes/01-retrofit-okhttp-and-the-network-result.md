...
```mermaid
flowchart TD
  A["suspend api call"] --> B{"What happened"}
  B -- "Server responded with error code" --> C["HttpException"]
  B -- "No response at all" --> D["IOException"]
  B -- "Body did not match the DTO" --> E["SerializationException"]
  B -- "Everything worked" --> F["Success"]
  C --> G["NetworkResult HttpError"]
  D --> H["NetworkResult NetworkError"]
  E --> I["NetworkResult SerializationError"]
  F --> J["NetworkResult Success"]
```
*How safeApiCall maps each of the three failure paths, plus success, onto a NetworkResult case.*
...
sequenceDiagram
  ...
  Code->>Auth: request
  Auth->>Log: request plus header
  Log->>Retry: request logged
  Retry->>Net: request sent
  Net-->>Retry: response
  Retry-->>Log: response
  Log-->>Auth: response logged
  Auth-->>Code: response
```
*A request travels down through auth, logging, then retry, and the response travels back up in reverse order.*
