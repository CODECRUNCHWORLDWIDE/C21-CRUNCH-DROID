...
```mermaid
flowchart LR
  Free["free flavor"] --> FreeDebug["freeDebug"]
  Free --> FreeRelease["freeRelease"]
  Pro["pro flavor"] --> ProDebug["proDebug"]
  Pro --> ProRelease["proRelease"]
  Debug["debug build type"] --> FreeDebug
  Debug --> ProDebug
  Release["release build type"] --> FreeRelease
  Release --> ProRelease
```
*Two orthogonal axes, flavor and build type, multiply into four buildable variants.*
...
```mermaid
flowchart LR
  A["Build fails red"] --> B["Read the failing task name"]
  B --> C["Map task to pipeline stage"]
  C --> D["Read the error at that stage"]
  D --> E["Apply the targeted fix"]
  E --> F["Rebuild and confirm green"]
```
*The senior debugging loop: task name to stage to cause to fix, instead of guessing from the error text alone.*
