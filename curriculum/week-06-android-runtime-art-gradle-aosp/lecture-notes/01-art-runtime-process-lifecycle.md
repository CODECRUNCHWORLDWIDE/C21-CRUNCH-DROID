...
stateDiagram-v2
  ...
  Created --> [*] : onDestroy
  Created --> [*] : OS kills process - no onDestroy guaranteed
```
*The Activity lifecycle as a state machine: the normal onCreate-to-onDestroy path, plus the abrupt exit an OS process kill can take from any background state.*
...
sequenceDiagram
  ...
  participant SysServer as system_server
  participant Zygote
  participant Proc as your process
  User->>Launcher: tap app icon
  Launcher->>SysServer: send start Activity intent
  SysServer->>Zygote: fork new process
  Zygote->>Proc: forked process begins
  Proc->>Proc: ActivityThread main starts Looper
  SysServer->>Proc: dispatch onCreate
  Proc->>Proc: onStart then onResume
```
*From tap to onscreen: the launcher's intent, the Zygote fork, and the OS dispatching your first lifecycle callbacks onto the new process's main Looper.*
