...
```mermaid
flowchart TD
  Root["Three chaos drills"] --> A["Drill A offline sync conflict"]
  Root --> B["Drill B FCM token rotation"]
  Root --> C["Drill C Play Integrity failure"]
  A --> A1["Tests conflict resolution ADR-0001 and ADR-0003"]
  B --> B1["Tests push pipeline resilience"]
  C --> C1["Tests graceful degradation ADR-0004"]
```
*Each of the three drills exercises a different load-bearing contract of the capstone.*
...
```mermaid
flowchart TD
  A["Write locally state PENDING"] --> B["Enqueue in the outbox"]
  B --> C["POST with message id as idempotency key"]
  C --> D["Server ack state SENT"]
  D --> E["Delivery receipt state DELIVERED"]
  E --> F["Read receipt state READ"]
  B -.offline.-> G["Stays PENDING until reconnect then drains in order"]
```
*The offline-first send pipeline: write locally, queue, then transition through SENT, DELIVERED, and READ.*
