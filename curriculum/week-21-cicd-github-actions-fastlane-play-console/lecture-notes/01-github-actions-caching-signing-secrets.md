...
```mermaid
flowchart TD
  A["CI signs bundle with upload key"] --> B["Play Console verifies upload key signature"]
  B --> C["Play strips the upload signature"]
  C --> D["Google re-signs with app signing key"]
  D --> E["Signed APK delivered to users"]
  F["Upload key lost"] --> G["Request reset from Google"]
  G --> A
```
*You sign with the upload key you hold; Google alone holds the app signing key that actually reaches users.*
...
```mermaid
flowchart TD
  A["PR or push to main"] --> B["ci job: unit tests, lint, Paparazzi"]
  T["Version tag pushed"] --> B
  B -->|pass| C{"Is this commit tagged"}
  B -->|fail| X["Merge or release blocked"]
  C -->|yes| D["release job starts"]
  C -->|no| Y["CI only, no release"]
  D --> E["Decode secrets into RUNNER_TEMP"]
  E --> F["Build signed bundle"]
  F --> G["Upload to Play internal track"]
```
*A tag only reaches the release job after ci passes; secrets stay scoped to that one job.*
