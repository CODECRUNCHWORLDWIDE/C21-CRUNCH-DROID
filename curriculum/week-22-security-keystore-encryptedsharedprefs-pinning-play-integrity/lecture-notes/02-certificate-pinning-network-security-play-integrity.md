...
```mermaid
flowchart TD
  A["Pin the leaf certificate SHA-256"] --> B["A year later the leaf expires and rotates"]
  B --> C["Installed apps still hold the old pin"]
  C --> D["New certificate fails the pin check"]
  D --> E["Connection refused app is bricked"]
  E --> F["Only fix is a forced update taking days"]
```
*Pinning the leaf without a backup pin turns a routine certificate rotation into a bricked app.*
...
sequenceDiagram
  participant Client
  participant Backend
  participant Google as Play Integrity API
  Client->>Backend: GET auth nonce
  Backend-->>Client: fresh single-use nonce
  Client->>Google: requestIntegrityToken nonce
  Google-->>Client: opaque token
  Client->>Backend: POST auth sign-in token
  Backend->>Google: decode token
  Google-->>Backend: verdict for app device account
  Backend-->>Client: allow or deny session
```
*The client only ferries an opaque token — decoding and the allow or deny decision happen on the backend.*
