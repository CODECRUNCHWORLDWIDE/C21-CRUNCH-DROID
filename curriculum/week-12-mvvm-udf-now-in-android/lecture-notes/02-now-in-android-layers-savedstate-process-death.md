...
```mermaid
flowchart TD
  UI["UI layer ViewModel and composables"] --> Domain["Domain layer optional use cases"]
  Domain --> Data["Data layer repositories"]
  Data --> Sources["Data sources Room Retrofit DataStore"]
```
*Dependencies point one way only: UI to domain to data, never back up.*
...
stateDiagram-v2
  [*] --> Running
  Running --> Running: Configuration change ViewModel survives
  Running --> Destroyed: Process death ViewModel destroyed
  Destroyed --> Running: App reopened new ViewModel SavedStateHandle restores
```
*Rotation leaves the ViewModel alone; only a saved SavedStateHandle slice survives a real process kill.*
