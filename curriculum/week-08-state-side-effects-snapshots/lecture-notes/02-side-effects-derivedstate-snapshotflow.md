...
stateDiagram-v2
    [*] --> Entered
    Entered --> Running : composable enters, launch coroutine
    Running --> Running : recompose, key unchanged
    Running --> Restarted : key changes, cancel then relaunch
    Restarted --> Running
    Running --> Cancelled : composable leaves
    Cancelled --> [*]
```
*LaunchedEffect ties a coroutine to enter, key change, and leave.*
...
```mermaid
flowchart LR
    Q["query MutableState reads"] --> F1["snapshotFlow emits String"]
    F1 --> D["debounce 300ms"]
    D --> DU["distinctUntilChanged"]
    DU --> FL["filter length at least 2"]
    FL --> FM["flatMapLatest runs search, cancels prior"]
    FM --> C["collect writes results state"]
```
*The debounced search pipeline: snapshotFlow lifts query changes into Week 5's Flow operators.*
